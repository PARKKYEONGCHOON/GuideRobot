...
import rospy
from robotsystem.msg import map
...
class mapPubsub:
    ...
    def __init__(self):
        ...
        self.Mappub = rospy.Publisher("/Self_map", map, queue_size=1)
        self.MsgMap = map()
    ...
    def Lidar_Scan_Callback(self,msg):
        ...
        for i in range(len(gl_range)):
            ...
            #print(gl_range)
        dictRange = dict(range = gl_range)
        tmpdict = dict(dictRange, **self.dictOdom)
            
        stampdict = {str(secs)+"."+ str(nsecs): tmpdict}
        #tmpstampdic[str(secs)+"."+ str(nsecs)] = tmpdict
            
        #self.mapdict = {'map': stampdict}
        self.mapdict = stampdict
        
        #tmpslamMapdic = {'map': tmpstampdic}
            
        #slamMap = mapdict
        #slamMaps = tmpslamMapdic
        ...
        sensorData = self.mapdict
        
        #print(self.mapdict)
        #sensorData = self.readJson('./Currentmapdata.json')
        #print(sensorData)
        ...
        if self.sw:
            ...
            self.pf = self.processSensorData(self.pf, sensorData, self.sw, plotTrajectory=True)
        else:
            self.pf = ParticleFilter(numParticles, ogParameters, smParameters)
            self.pf = self.processSensorData(self.pf, sensorData, self.sw, plotTrajectory=True)
            self.sw = True
        
    def processSensorData(self,pf, sensorData, sw, plotTrajectory = True):
        ...
        key = sorted(sensorData.keys())
        print(key)
        self.pfCount += 1
        print(self.pfCount)
        pf.updateParticles(sensorData[key[0]], self.pfCount)
        ...
        print(ogMap)
            ...
            # image = ogMap.astype(np.uint8)
            # print(image)
            # image = np.array(image).flatten().tolist()
            
            # self.MsgMap.data = image
            
        if len(ogMap) == 0:
            pass
        else:
            plt.imsave('./Output/' + 'matimg.png',ogMap, cmap='gray')
            image = cv2.imread('./Output/' + 'matimg.png',cv2.cv2.COLOR_BGR2RGB)
            h,w,r = image.shape
            ...
            image = np.array(image).flatten().tolist()
            self.MsgMap.data = image
            self.Mappub.publish(self.MsgMap)
...
def main():
    pubsub = mapPubsub()
    rospy.init_node('FastSlamV2', anonymous=True)
    try:
        rospy.spin()
...
#     rospy.init_node("FastSlam")
    ...
    #     #plt.close()
    #     #sensorData = readJson("./JSON/Currentmapdata.json")
    #     if sensorData != None:
    #         sensorData = pubsub.mapdict
    #         print(sensorData)
    #         numSamplesPerRev = len(sensorData[list(sensorData)[0]]['range'])  # Get how many points per revolution
    #         initXY = sensorData[sorted(sensorData.keys())[0]]
    #         numParticles = 1
